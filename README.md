...
Disable [Action Scheduler's](https://actionscheduler.org) default queue runner, by removing it from the `'action_scheduler_run_queue'` hook. Handy for temporarily disabling scheduled events, like on a dev or test site, or on live sites when running Action Scheduler via [WP CLI](https://actionscheduler.org/wp-cli/).
